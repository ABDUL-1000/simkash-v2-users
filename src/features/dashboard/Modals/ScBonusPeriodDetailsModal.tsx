...
export interface BonusPeriodRecord {
  period: string;
  startDate: string;
  endDate: string;
  target: number;
...
interface ScBonusPeriodDetailsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  periodData?: BonusPeriodRecord | null;
  onDownloadReport?: () => void;
}

export function ScBonusPeriodDetailsModal({
  open,
  onOpenChange,
  periodData,
  onDownloadReport,
}: ScBonusPeriodDetailsModalProps) {
  ...
  return (
    ...
    >
      <div className="space-y-4 pt-1 text-xs max-h-[80vh] overflow-y-auto pr-1">
        ...
        {/* Missed Reasons & Encouragement Box (Image 1) */}
        {isMissed && (
          <>
            ...
            <div className="rounded-2xl bg-[#F8FAFC] p-3 text-xs text-[#0F152A] font-semibold text-center">
              {periodData.bounceBackNote || "You bounced back in March with 421 activations! Keep pushing. 💪"}
            </div>
          </>
        )}

        {/* Achieved Charts & Top APs (Image 5) */}
        {isAchieved && (
          <>
              ...
              <div className="space-y-2 rounded-2xl border border-[#E2ECF6] bg-white p-3">
                {defaultTopAps.map((ap, idx) => (
                  <div key={idx} className="flex items-center justify-between text-xs py-1 border-b last:border-b-0 border-[#F8FAFC]">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-[#8C909B] font-medium">{ap.acts} acts</span>
                      <span className="font-extrabold text-[#10B981]">{ap.commission}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[#E2ECF6]">
          {isInProgress ? (
            <button
              ...
              onClick={() => {
                onOpenChange(false);
                navigate(appPaths.scBonusTracker);
              }}
              className="font-extrabold text-[#2563EB] text-xs hover:underline mx-auto"
            >
              Return to Tracker →
            </button>
          ) : (
            <>
              {isAchieved && (
                <button
                  ...
                  onClick={() => {
                    onDownloadReport?.();
                    alert(`Report for ${periodData.period} downloaded!`);
                  }}
                  className="flex items-center gap-1.5 text-xs font-bold text-[#2563EB] hover:underline"
                >
                  <Download className="size-3.5" />
                  <span>Download Period Report</span>
                </button>
              )}
              <div className={!isAchieved ? "ml-auto" : ""}>
                <button
                  type="button"
