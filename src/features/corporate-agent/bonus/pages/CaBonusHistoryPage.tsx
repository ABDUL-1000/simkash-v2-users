...
import { Coins, Percent, Trophy, XCircle } from "lucide-react";
...
export function CaBonusHistoryPage() {
  ...
  const myBonusRecords: CaBonusPeriodRecord[] = [
    {
      period: "Jun 2026",
  ...
  const handleOpenDetails = (rec: CaBonusPeriodRecord) => {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      <PageHeader
        ...
        description="All your past bonus periods and AP network bonus records"
        actions={[
          {
            key: "back",
            label: "Back to Tracker",
            onClick: () => navigate(appPaths.caBonusTracker),
            variant: "outline",
          },
          {
            key: "export-history",
            ...
            {(["All Time", "This Year", "Last 6 Months"] as const).map((filter) => (
              <button
                ...
                className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${
                  timeFilter === filter ? "bg-[#0F152A] text-white" : "bg-white border border-[#E2ECF6] text-[#66738C] hover:bg-[#F8FAFC]"
                }`}
              >
                {filter}
              </button>
                  ...
                  {myBonusRecords.map((row, idx) => (
                    <tr key={idx} className="hover:bg-[#F8FAFC]">
                      ...
                      <td className="py-4">
                        <span
                          className={`rounded-full px-3 py-0.5 text-[10px] font-bold ${
                            row.status === "Achieved" ? "bg-[#E0E7FF] text-[#4F46E5]" : row.status === "In Progress" ? "bg-[#EFF6FF] text-[#2563EB]" : "bg-[#FFF7F8] text-[#EF4444]"
                          }`}
                        >
                          {row.status}
                        </span>
                      ...
                      <td className="py-4 text-right">
                        <button type="button" onClick={() => handleOpenDetails(row)} className="font-bold text-[#2563EB] hover:underline text-xs">
                          View Details
                        </button>
                      </td>
                    </tr>
          ...
          <div className="rounded-3xl border border-[#E2ECF6] bg-white p-6 shadow-xs space-y-4">
            <div className="overflow-x-auto">
              <table className="w-full min-w-180 text-left text-xs">
                ...
                <tbody className="divide-y divide-[#E2ECF6]">
                  {apMatrix.map((ap, idx) => (
                    <tr key={idx} onClick={() => handleOpenApMatrixHistory(ap.name)} className="cursor-pointer hover:bg-[#F8FAFC] transition">
                      <td className="py-4 font-bold text-[#0F152A] hover:text-[#2563EB]">{ap.name}</td>
                      <td className="py-4 text-center"><DotStatus status={ap.jun} /></td>
                      <td className="py-4 text-center"><DotStatus status={ap.may} /></td>
                      <td className="py-4 text-center"><DotStatus status={ap.apr} /></td>
                      <td className="py-4 text-center"><DotStatus status={ap.mar} /></td>
                      <td className="py-4 font-extrabold text-[#0F152A]">{ap.earned}</td>
                      <td className="py-4 text-right font-extrabold">
                        {ap.rate === "pending" ? <span className="text-[#2563EB]">pending</span> : <span className="text-[#0F152A]">{ap.rate}</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pt-3 border-t border-[#E2ECF6]">
              ...
              <button type="button" onClick={() => alert("Loading 15 more APs...")} className="text-xs font-black text-[#2563EB] hover:underline">
                Load more (15 APs) →
              </button>
            </div>
